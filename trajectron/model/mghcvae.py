...
class MultimodalGenerativeHCVAE(object):
    def __init__(self,
                 env,
                 node_type,
                 model_registrar,
    ...
    def create_node_models(self):
        ...
        ################################
        self.latent_c = DiscreteLatent(self.hyperparams, self.device)
        ...
        c_size = self.hyperparams['N'] * self.hyperparams['K']

        if self.hyperparams['p_c_x_MLP_dims'] is not None:
            self.add_submodule(self.node_type + '/p_c_x',
                               model_if_absent=nn.Linear(x_size, self.hyperparams['p_c_x_MLP_dims']))
            hx_size = self.hyperparams['p_c_x_MLP_dims']
        else:
            hx_size = x_size

        self.add_submodule(self.node_type + '/hx_to_c',
                           model_if_absent=nn.Linear(hx_size, self.latent_c.c_dim))

        if self.hyperparams['q_c_xy_MLP_dims'] is not None:
            self.add_submodule(self.node_type + '/q_c_xy',
                               #                                           Node Future Encoder
                               model_if_absent=nn.Linear(x_size + 4 * self.hyperparams['enc_rnn_dim_future'],
                                                         self.hyperparams['q_c_xy_MLP_dims']))
            hxy_size = self.hyperparams['q_c_xy_MLP_dims']
        else:
            #                           Node Future Encoder
            hxy_size = x_size + 4 * self.hyperparams['enc_rnn_dim_future']

        self.add_submodule(self.node_type + '/hxy_to_c',
                           model_if_absent=nn.Linear(hxy_size, self.latent_c.c_dim))

        ####################
        #   Decoder LSTM   #
        ####################
        if self.hyperparams['incl_robot_node']:
            decoder_input_dims = self.pred_state_length + self.robot_state_length + c_size + x_size
        else:
            decoder_input_dims = self.pred_state_length + c_size + x_size
        ...
        self.add_submodule(self.node_type + '/decoder/initial_h',
                           model_if_absent=nn.Linear(c_size + x_size, self.hyperparams['dec_rnn_dim']))
    ...
    def set_annealing_params(self):
        ...
        self.create_new_scheduler(name='latent_c.temp',
                                  annealer=exp_anneal,
                                  annealer_kws={
        ...
        self.create_new_scheduler(name='latent_c.c_logit_clip',
                                  annealer=sigmoid_anneal,
                                  annealer_kws={
                                      'start': self.hyperparams['c_logit_clip_start'],
                                      'finish': self.hyperparams['c_logit_clip_final'],
                                      'center_step': self.hyperparams['c_logit_clip_crossover'],
                                      'steps_lo_to_hi': self.hyperparams['c_logit_clip_crossover'] / self.hyperparams[
                                          'c_logit_clip_divisor']
                                  },
                                  creation_condition=self.hyperparams['use_c_logit_clipping'])
    ...
    def q_c_xy(self, mode, x, y_e) -> torch.Tensor:
        r"""
        ...
        if self.hyperparams['q_c_xy_MLP_dims'] is not None:
            dense = self.node_modules[self.node_type + '/q_c_xy']
        ...
        to_latent_c = self.node_modules[self.node_type + '/hxy_to_c']
        return self.latent_c.dist_from_h(to_latent_c(h), mode)

    def p_c_x(self, mode, x):
        r"""
        ...
        """
        if self.hyperparams['p_c_x_MLP_dims'] is not None:
            dense = self.node_modules[self.node_type + '/p_c_x']
        ...
        to_latent_c = self.node_modules[self.node_type + '/hx_to_c']
        return self.latent_c.dist_from_h(to_latent_c(h), mode)
    ...
    def p_y_xc(self, mode, x, x_nr_t, y_r, n_s_t0, c_stacked, prediction_horizon,
               num_samples, num_components=1, gmm_mode=False):
        r"""
        .. math:: p_\psi(\mathbf{y}_i \mid \mathbf{x}_i, c)
        ...
        :param c_stacked: Stacked latent state. [num_samples_c * num_samples_gmm, bs, latent_state]
        ...
        c = torch.reshape(c_stacked, (-1, self.latent_c.c_dim))
        cx = torch.cat([c, x.repeat(num_samples * num_components, 1)], dim=1)

        cell = self.node_modules[self.node_type + '/decoder/rnn_cell']
        initial_h_model = self.node_modules[self.node_type + '/decoder/initial_h']

        initial_state = initial_h_model(cx)
        ...
        if self.hyperparams['incl_robot_node']:
            ...
            input_ = torch.cat([cx, a_0.repeat(num_samples * num_components, 1)], dim=1)
        ...
        for j in range(ph):
            ...
            if num_components > 1:
                if mode == ModeKeys.PREDICT:
                    log_pis.append(self.latent_c.p_dist.logits.repeat(num_samples, 1, 1))
                else:
                    log_pis.append(self.latent_c.q_dist.logits.repeat(num_samples, 1, 1))
            else:
                log_pis.append(
            ...
            if self.hyperparams['incl_robot_node']:
                dec_inputs = [cx, a_t, y_r[:, j].repeat(num_samples * num_components, 1)]
            else:
                dec_inputs = [cx, a_t]
    ...
    def encoder(self, mode, x, y_e, num_samples=None):
        ...
        :return: tuple(c, kl_obj)
            WHERE
            - c: Samples from the latent space.
        ...
        self.latent_c.q_dist = self.q_c_xy(mode, x, y_e)
        self.latent_c.p_dist = self.p_c_x(mode, x)

        c = self.latent_c.sample_q(sample_ct, mode)

        if mode == ModeKeys.TRAIN:
            kl_obj = self.latent_c.kl_q_p(self.log_writer, '%s' % str(self.node_type), self.curr_iter)
            if self.log_writer is not None:
    ...
    def decoder(self, mode, x, x_nr_t, y, y_r, n_s_t0, c, labels, prediction_horizon, num_samples):
        """
        Decoder of the CVAE.
        ...
        y_dist = self.p_y_xc(mode, x, x_nr_t, y_r, n_s_t0, c,
                             prediction_horizon, num_samples, num_components=num_components)
        log_p_yt_xc = torch.clamp(y_dist.log_prob(labels), max=self.hyperparams['log_p_yt_xc_max'])
        if self.hyperparams['log_histograms'] and self.log_writer is not None:
            self.log_writer.add_histogram('%s/%s' % (str(self.node_type), 'log_p_yt_xc'), log_p_yt_xc, self.curr_iter)

        log_p_y_xc = torch.sum(log_p_yt_xc, dim=2)
        return log_p_y_xc
    ...
    def train_loss(self,
        ...
        c, kl = self.encoder(mode, x, y_e)
        log_p_y_xc = self.decoder(mode, x, x_nr_t, y, y_r, n_s_t0, c,
                                  labels,  # Loss is calculated on unstandardized label
                                  prediction_horizon,
                                  self.hyperparams['k'])

        log_p_y_xc_mean = torch.mean(log_p_y_xc, dim=0)  # [nbs]
        log_likelihood = torch.mean(log_p_y_xc_mean)

        mutual_inf_q = mutual_inf_mc(self.latent_c.q_dist)
        mutual_inf_p = mutual_inf_mc(self.latent_c.p_dist)

        ELBO = log_likelihood - self.kl_weight * kl + 1. * mutual_inf_p
        loss = -ELBO

        if self.hyperparams['log_histograms'] and self.log_writer is not None:
            self.log_writer.add_histogram('%s/%s' % (str(self.node_type), 'log_p_y_xc'),
                                          log_p_y_xc_mean,
                                          self.curr_iter)
        ...
        if self.log_writer is not None:
            ...
            if self.hyperparams['log_histograms']:
                self.latent_c.summarize_for_tensorboard(self.log_writer, str(self.node_type), self.curr_iter)
        return loss
    ...
    def eval_loss(self,
        ...
        c, _ = self.encoder(mode, x, y_e)  # [k_eval, nbs, N*K]
        c = self.latent_c.sample_p(1, mode, full_dist=True)
        y_dist, _ = self.p_y_xc(ModeKeys.PREDICT, x, x_nr_t, y_r, n_s_t0, c,
    ...
    def predict(self,
        ...
        :param prediction_horizon: Number of prediction timesteps.
        :param num_samples: Number of samples from the latent space.
        :param c_mode: If True: Select the most likely latent state.
        :param gmm_mode: If True: The mode of the GMM is sampled.
        :param all_c_sep: Samples each latent mode individually without merging them into a GMM.
        ...
        self.latent_c.p_dist = self.p_c_x(mode, x)
        c, num_samples, num_components = self.latent_c.sample_p(num_samples,
                                                              mode,
                                                              most_likely_c=c_mode,
                                                              full_dist=full_dist,
                                                              all_c_sep=all_c_sep)

        _, our_sampled_future = self.p_y_xc(mode, x, x_nr_t, y_r, n_s_t0, c,
                                            prediction_horizon,
                                            num_samples,
                                            num_components,
                                            gmm_mode)
