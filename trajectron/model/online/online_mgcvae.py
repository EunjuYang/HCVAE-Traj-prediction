...
from model.mgcvae import MultimodalGenerativeHCVAE
from environment.scene_graph import DirectedEdge
from environment.node_type import NodeType


class OnlineMultimodalGenerativeHCVAE(MultimodalGenerativeHCVAE):
    def __init__(self,
                 env,
                 node,
                 model_registrar,
    ...
    def encoder_forward(self, inputs, inputs_st, inputs_np, robot_present_and_future=None, maps=None):
        ...
        self.latent.p_dist = self.p_c_x(mode, self.x)
    ...
    def decoder_forward(self, prediction_horizon,
        ...
        c, num_samples, num_components = self.latent.sample_p(num_samples,
                                                              mode,
                                                              most_likely_c=c_mode,
                                                              full_dist=full_dist,
                                                              all_c_sep=all_c_sep)

        y_dist, our_sampled_future = self.p_y_xc(mode, self.x, x_nr_t, y_r, self.n_s_t0, c,
                                                 prediction_horizon,
                                                 num_samples,
                                                 num_components,
                                                 gmm_mode)
