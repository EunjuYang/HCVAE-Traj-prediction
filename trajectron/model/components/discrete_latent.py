...
class DiscreteLatent(object):
    def __init__(self, hyperparams, device):
        ...
        self.c_dim = hyperparams['N'] * hyperparams['K']
        ...
        self.c_logit_clip = None  # filled in by MultimodalGenerativeCVAE.set_annealing_params
    ...
    def dist_from_h(self, h, mode):
        ...
        if self.c_logit_clip is not None and mode == ModeKeys.TRAIN:
            c = self.c_logit_clip
    ...
    def sample_q(self, num_samples, mode):
        ...
        c_NK = torch.from_numpy(self.all_one_hot_combinations(self.N, self.K)).float().to(self.device).repeat(num_samples, bs)
        return torch.reshape(c_NK, (num_samples * num_components, -1, self.c_dim))

    def sample_p(self, num_samples, mode, most_likely_c=False, full_dist=True, all_c_sep=False):
        num_components = 1
        if full_dist:
            bs = self.p_dist.probs.size()[0]
            c_NK = torch.from_numpy(self.all_one_hot_combinations(self.N, self.K)).float().to(self.device).repeat(num_samples, bs)
            ...
            bs = self.p_dist.probs.size()[0]
            c_NK = torch.from_numpy(self.all_one_hot_combinations(self.N, self.K)).float().to(self.device).repeat(1, bs)
            k = self.K ** self.N
            num_samples = k
        elif most_likely_c:
            # Sampling the most likely c from p(c|x).
            eye_mat = torch.eye(self.p_dist.event_shape[-1], device=self.device)
            argmax_idxs = torch.argmax(self.p_dist.probs, dim=2)
            c_NK = torch.unsqueeze(eye_mat[argmax_idxs], dim=0).expand(num_samples, -1, -1, -1)
            k = num_samples
        else:
            c_NK = self.p_dist.sample((num_samples,))
            k = num_samples

        if mode == ModeKeys.PREDICT:
            return torch.reshape(c_NK, (k, -1, self.N * self.K)), num_samples, num_components
        else:
            return torch.reshape(c_NK, (k, -1, self.N * self.K))
    ...
    def q_log_prob(self, c):
        k = c.size()[0]
        c_NK = torch.reshape(c, [k, -1, self.N, self.K])
        return torch.sum(self.q_dist.log_prob(c_NK), dim=2)

    def p_log_prob(self, c):
        k = c.size()[0]
        c_NK = torch.reshape(c, [k, -1, self.N, self.K])
        return torch.sum(self.p_dist.log_prob(c_NK), dim=2)
    ...
    def summarize_for_tensorboard(self, log_writer, prefix, curr_iter):
        log_writer.add_histogram(prefix + "/latent/p_c_x", self.p_dist.probs, curr_iter)
        log_writer.add_histogram(prefix + "/latent/q_c_xy", self.q_dist.probs, curr_iter)
        log_writer.add_histogram(prefix + "/latent/p_c_x_logits", self.p_dist.logits, curr_iter)
        log_writer.add_histogram(prefix + "/latent/q_c_xy_logits", self.q_dist.logits, curr_iter)
        if self.c_dim <= 9:
            for i in range(self.N):
                for j in range(self.K):
                    log_writer.add_histogram(prefix + "/latent/q_c_xy_logit{0}{1}".format(i, j),
